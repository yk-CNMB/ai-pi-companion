...
def init_model():
    ...
    ms = scan_models()
    # 优先找 Hiyori，如果找不到就用列表第一个，防止崩溃
    t = next((m for m in ms if "hiyori" in m['id'].lower()), ms[0] if ms else None)
    if t:
        CURRENT_MODEL = t
        print(f"🤖 模型初始化: {t['name']}")
    else:
        print("⚠️ 未找到可用模型，请先下载或上传")

init_model()

# --- 5. 语音合成引擎 (三大引擎集合) ---

# 引擎 A: OpenAI 格式 (Fish Audio / GPT-SoVITS)
