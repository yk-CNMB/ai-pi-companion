# =======================================================================
# Pico AI Server - app.py (全功能 + 语音扫描增强)
# =======================================================================
...
def scan_local_voices():
    """扫描本地 Piper 模型"""
    voices = []
    print(f"🔍 [TTS] 正在扫描语音目录: {VOICES_DIR}")
    ...
    print(f"🔍 [TTS] 找到 {len(onnx_files)} 个 .onnx 文件")
    ...
    for onnx in onnx_files:
        ...
        voices.append({"id": rel_path, "name": f"🏠 {name} (本地)"})
        print(f"  -> 添加语音: {name} ({rel_path})")
        
    return voices

def run_piper_tts(text, model_rel_path, output_path):
    # 拼接完整路径
    model_path = os.path.join(VOICES_DIR, model_rel_path)
    ...
    try:
        print(f"🎤 [Piper] 生成中... 使用模型: {model_rel_path}")
        cmd = [PIPER_BIN, "--model", model_path, "--output_file", output_path]
        # 必须使用 communicate 传入 stdin
...
def on_get_data():
    ...
    # 【核心】调用扫描函数并合并列表
    local_voices = scan_local_voices()
