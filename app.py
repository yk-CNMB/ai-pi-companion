...
# 修复了 glob 缺失，语法错误，支持 Piper/Edge
# =======================================================================
import os
import json
import uuid
...
def on_login(d):
    ...
    emit('response', {'text': f"[HAPPY] 嗨 {u}!", 'sender': 'Pico', 'emotion': 'HAPPY'}, to=request.sid)
...
def on_get_data():
    ...
    if os.path.exists(VOICES_DIR):
        for onnx in glob.glob(os.path.join(VOICES_DIR, "*.onnx")):
            ...
            voices.append({"id": os.path.basename(onnx), "name": f"🏠 {name} (本地)"})
