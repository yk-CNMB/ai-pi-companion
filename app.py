# =======================================================================
# Pico AI Server - app.py (Python 3.13 兼容 / 原生线程 / Piper集成)
# =======================================================================
import os
import json
import uuid
...
# 使用原生线程模式 (Threading) 适配 Python 3.13
...
def on_get_data():
    ...
    if os.path.exists(VOICES_DIR):
        # 递归扫描所有 onnx
        all_files = glob.glob(os.path.join(VOICES_DIR, "**", "*.onnx"), recursive=True)
        print(f"🔍 找到 {len(all_files)} 个模型")
        
        for onnx in all_files:
            # 使用相对路径作为 ID，防止子目录问题
            ...
            if os.path.exists(txt_path):
                try:
                    name = open(txt_path, 'r').read().strip()
                except: pass
