# =======================================================================
# Pico AI Server - app.py (语音强力诊断版)
# =======================================================================
import os, json, uuid, asyncio, time, glob, shutil, re, zipfile, subprocess, requests, traceback
import eventlet
...
# --- 核心函数 ---
def load_user_memories(u): return [] # 前端管理
# 模型管理 (精简版，逻辑不变)
CURRENT_MODEL = {"id": "default", "path": "", "persona": "", "voice": "zh-CN-XiaoxiaoNeural", "rate": "+0%", "pitch": "+0Hz", "api_url":"", "api_key":"", "model_id":""}
...
def run_openai_tts(text, api_url, api_key, model_id, output_path):
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"🐟 [Fish] 成功保存到: {output_path} (大小: {os.path.getsize(output_path)} bytes)")
            return True
        else:
            print(f"🐟 [Fish] API 错误: {resp.status_code} - {resp.text}")
            return False
