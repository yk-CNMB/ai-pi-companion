...
def run_openai_tts(text, api_url, api_key, model_id, output_path):
    try:
        # 智能补全 Fish Audio URL
        target_url = api_url
        if "fish.audio" in api_url and not api_url.endswith("/v1/audio/speech") and not api_url.endswith("/v1/tts"):
             # 尝试原生
             target_url = "https://api.fish.audio/v1/tts"
        elif not api_url.endswith("/speech") and not api_url.endswith("/tts"):
