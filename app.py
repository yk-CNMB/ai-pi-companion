# =======================================================================
# Pico AI Server - app.py (Piper 纯净版)
...
def bg_tts(text, voice, rate, pitch, room=None, sid=None):
    ...
    # 1. 优先尝试 Piper (如果 voice 是 .onnx 结尾)
    ...
    if not success:
        ...
        # 如果 voice 还是 .onnx (说明Piper失败了)，强制改回晓晓
        safe_voice = voice
        if ".onnx" in voice or "Neural" not in voice:
...
def on_message(d):
    ...
    if "/管理员" in msg:
        if sender.lower() == "yk":
            ...
            emit('admin_unlocked')
            emit('system_message', {'text': f"👑 管理员已认证"}, to=sid)
        else:
...
def on_get_data():
    ...
    # 只要 static/voices 目录下有 .onnx 文件，就会自动添加到列表
