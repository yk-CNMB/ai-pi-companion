# =======================================================================
# Pico AI Server - app.py (Fish Audio 列表修复版)
# =======================================================================
...
def run_openai_tts(text, api_url, api_key, model_id, output_path):
    try:
        # 默认补全 Fish Audio
        if not api_url: api_url = "https://api.fish.audio/v1/tts"
        
        # 自动识别 Fish Audio 并切换到原生接口 (更稳定)
...
def on_get_data():
    # 【核心修复】手动加入 Fish Audio 选项
    voices = [
        {"id":"fish-audio", "name":"🐟 Fish Audio (自定义)"}, # 👈 加上这行
...
def on_save_settings(d):
    ...
    try: 
        d['scale'] = float(d['scale']); d['x'] = float(d['x']); d['y'] = float(d['y'])
    except: pass
