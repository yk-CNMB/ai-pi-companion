...
def get_model_config(mid):
    ...
    # 默认配置
    data = {
        "persona":f"你是{mid}。", "voice":"zh-CN-XiaoxiaoNeural",
        "rate":"+0%", "pitch":"+0Hz", 
        "scale": 0.5, "x": 0.5, "y": 0.5, # 默认位置
        "api_url":"", "api_key":"", "model_id":""
    }
    if os.path.exists(p):
        try: 
            loaded = json.load(open(p))
            # 确保读取的是数字类型，防止前端发来字符串导致计算错误
            if 'scale' in loaded: loaded['scale'] = float(loaded['scale'])
            if 'x' in loaded: loaded['x'] = float(loaded['x'])
            if 'y' in loaded: loaded['y'] = float(loaded['y'])
            data.update(loaded)
        except: pass
    return data
...
def scan_models():
    ...
    for j in glob.glob(os.path.join(MODELS_DIR, "**", "*.model3.json"), recursive=True):
        ...
        ms.append({
            "id": mid, "name": mid.capitalize(), "path": "/"+os.path.relpath(j, BASE_DIR).replace("\\","/"), **cfg
        })
...
def on_login(d):
    ...
    # 【关键】发送 current_model 给前端，前端会用它来初始化位置
