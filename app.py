# =======================================================================
# Pico AI Server - app.py (全引擎版)
# 支持: Edge-TTS | Piper | Sherpa-Onnx (GlaDOS)
# =======================================================================
...
# 尝试导入 sherpa_onnx，如果没装也不报错（防止脚本还没跑完就崩）
try: import sherpa_onnx
except ImportError: sherpa_onnx = None
...
# --- Sherpa 引擎初始化 (专门服务 GlaDOS) ---
glados_tts = None
def init_glados():
    global glados_tts
    if not sherpa_onnx: return
    glados_path = os.path.join(VOICES_DIR, "glados")
    model_file = os.path.join(glados_path, "en_US-glados.onnx")
    tokens_file = os.path.join(glados_path, "tokens.txt")
            ...
            cfg = sherpa_onnx.OfflineTtsConfig(
                model=sherpa_onnx.OfflineTtsModelConfig(
                    vits=sherpa_onnx.OfflineTtsVitsModelConfig(
                        model=model_file,
                        tokens=tokens_file,
                        data_dir=os.path.join(glados_path, "espeak-ng-data"),
                    ),
                    provider="cpu",
                    num_threads=2
                )
            )
            glados_tts = sherpa_onnx.OfflineTts(cfg)
            print("✅ GlaDOS (Sherpa) 引擎就绪")
        except Exception as e:
            print(f"❌ GlaDOS 加载失败: {e}")

# 启动时尝试加载
threading.Thread(target=init_glados).start()
...
def run_piper_tts(text, model_file, output_path):
    ...
    try:
        subprocess.run(f'echo "{text}" | "{PIPER_BIN}" --model "{os.path.join(VOICES_DIR, model_file)}" --output_file "{output_path}"', shell=True, check=True)
        return True
    except: return False
...
def bg_tts(text, voice, rate, pitch, room=None, sid=None):
    ...
    # 1. Sherpa (GlaDOS)
    if voice == "glados-sherpa" and glados_tts:
        try:
            print("🐟 调用 GlaDOS 引擎...")
            audio = glados_tts.generate(clean, sid=0, speed=1.0)
            sf.write(os.path.join(AUDIO_DIR, f"{fname}.wav"), audio.samples, audio.sample_rate)
            success=True; url=f"/static/audio/{fname}.wav"
        except Exception as e: print(f"GlaDOS Error: {e}")
...
def on_login(d):
    ...
    welcome = f"[HAPPY] Hi {u}! I am running."
...
def on_message(d):
    ...
    try:
        if not chatroom_chat: init_chatroom()
        mem_ctx = f" (Memory: {', '.join(user_memories)})" if user_memories else ""
        resp = chatroom_chat.send_message(f"[{sender} says{mem_ctx}]: {msg}")
...
def on_get_data():
    voices = [{"id":"zh-CN-XiaoxiaoNeural","name":"☁️ 晓晓"}]
    # 添加 Piper 模型
    ...
    # 【新增】添加 GlaDOS (Sherpa) 选项
    if os.path.exists(os.path.join(VOICES_DIR, "glados/en_US-glados.onnx")):
        voices.append({"id": "glados-sherpa", "name": "🤖 GlaDOS (Sherpa)"})
    
    emit('studio_data', {'models': scan_models(), 'current_id': CURRENT_MODEL['id'], 'voices': voices})
# ... (switch_model, save_settings, delete_model, download_model 等保持不变，请保留原逻辑) ...
# 为了确保完整性，这里补充剩下的
