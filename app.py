# =======================================================================
# Pico AI Server - 终极全功能版 (MAX)
# 包含：双模TTS、人设保护、完整语音列表、文件管理
# =======================================================================
import os
import json
import uuid
...
app.config['SECRET_KEY'] = 'pico_secret_key_forever'
...
socketio = SocketIO(app, 
    ...
    ping_timeout=60, 
    ping_interval=25,
    max_http_buffer_size=50*1024*1024 # 50MB 缓冲
)
...
DEFAULT_INSTRUCTION = """
【指令】
请在回复开头标记心情：[HAPPY], [ANGRY], [SAD], [SHOCK], [NORMAL]。
"""
    ...
    if os.path.exists(p):
        try:
            with open(p, "r", encoding="utf-8") as f: 
                loaded = json.load(f)
                # ★★★ 关键：如果文件里有人设，就完全信任文件，不追加指令 ★★★
                # 只有当 persona 为空时，才使用默认值
...
def run_edge_tts_cmd(text, output_path, voice, rate, pitch):
    """方式1：命令行调用 (最稳)"""
    try:
        cmd = ["edge-tts", "--text", text, "--write-media", output_path, "--voice", voice, "--rate", rate, "--pitch", pitch]
...
def bg_tts_task(text, voice, rate, pitch, room=None, sid=None):
    ...
    # 优先尝试命令行
    if not run_edge_tts_cmd(clean, out_path, voice, rate, pitch):
        # 失败则尝试 Python 库
        run_edge_tts_python(clean, out_path, voice, rate, pitch)

    if os.path.exists(out_path) and os.path.getsize(out_path) > 0:
        url = f"/static/audio/{fname}"
        payload = {'audio': url}
        logging.info(f"🔊 Audio Generated: {url}")
...
def on_get_data():
    # 完整的 Edge-TTS 推荐列表
    voices = [
        {"id":"zh-CN-XiaoxiaoNeural", "name":"🇨🇳 晓晓 (默认女声)"},
        {"id":"zh-CN-YunxiNeural", "name":"🇨🇳 云希 (少年音)"},
        {"id":"zh-CN-YunjianNeural", "name":"🇨🇳 云健 (新闻男)"},
        {"id":"zh-CN-XiaoyiNeural", "name":"🇨🇳 晓伊 (可爱)"},
        {"id":"zh-TW-HsiaoChenNeural", "name":"🇹🇼 晓臻 (台湾女)"},
        {"id":"zh-HK-HiuMaanNeural", "name":"🇭🇰 晓曼 (粤语)"},
        {"id":"en-US-AnaNeural", "name":"🇺🇸 Ana (英文女)"},
        {"id":"en-US-GuyNeural", "name":"🇺🇸 Guy (英文男)"},
        {"id":"ja-JP-NanamiNeural", "name":"🇯🇵 七海 (日语女)"},
        {"id":"ja-JP-KeitaNeural", "name":"🇯🇵 圭太 (日语男)"}
    ]
    emit('studio_data', {
        'models': scan_models(), 
        'current_id': CURRENT_MODEL['id'], 
        'voices': voices, # 把语音列表还给前端
