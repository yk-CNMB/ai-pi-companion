# =======================================================================
# Pico AI Server - app.py (语音列表修复版)
# =======================================================================
import os, json, uuid, asyncio, time, glob, shutil, re, zipfile, subprocess, threading
import edge_tts
...
def run_piper_tts(text, model_file, output_path):
    # 兼容：如果 voice 包含路径，取文件名
    model_filename = os.path.basename(model_file)
    model_path = os.path.join(VOICES_DIR, model_filename)
    
    if not os.path.exists(PIPER_BIN): 
        print("❌ Piper 引擎未找到"); return False
    if not os.path.exists(model_path):
        print(f"❌ 模型未找到: {model_path}"); return False
...
def bg_tts(text, voice, rate, pitch, room=None, sid=None):
    ...
    if not success:
        ...
        except Exception as e: print(f"❌ TTS 失败: {e}")
...
def on_save_settings(d):
    ...
    # 只保存必要字段
    updated = save_model_config(d['id'], {k: d[k] for k in ['persona','voice','rate','pitch','scale','x','y'] if k in d})
