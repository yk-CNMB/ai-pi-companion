# =======================================================================
# Pico AI Server - app.py (绝对完整版)
# 包含: Fish Audio | Piper 本地语音 | Edge-TTS | 聊天室 | 管理员 | 完整记忆
# =======================================================================
import os
import json
import uuid
...
def run_openai_tts(text, api_url, api_key, model_id, output_path):
    try:
        ...
        print(f"📡 [Fish] 调用外部 TTS: {model_id}")
        headers = {
        ...
        print(f"❌ [Fish] Request Failed: {e}")
        return False

# 引擎 B: 本地 Piper (之前丢失的部分！已找回)
def run_piper_tts(text, model_file, output_path):
    ...
    if not os.path.exists(PIPER_BIN): 
        ...
        return False
    try:
        print(f"🏠 [Piper] 本地生成中: {model_file}")
...
def bg_tts(text, voice, rate, pitch, api_url, api_key, model_id, room=None, sid=None):
    ...
    if api_url and api_key and model_id:
        ...
        if run_openai_tts(clean, api_url, api_key, model_id, out_path):
            ...
            print("⚠️ External TTS failed, trying next...")

    # 2. 其次尝试本地 Piper (如果 voice 是 .onnx 文件)
    if not success and voice and voice.endswith(".onnx"):
         wav_path = os.path.join(AUDIO_DIR, f"{fname}.wav")
         if run_piper_tts(clean, voice, wav_path):
             success = True
             url = f"/static/audio/{fname}.wav"
         else:
             print("⚠️ Piper failed, trying fallback...")

    # 3. 最后兜底 Edge-TTS
    if not success:
        out_path = os.path.join(AUDIO_DIR, f"{fname}.mp3")
        # 如果 voice 是无效值（比如之前的 fish-audio 遗留），强制改回晓晓
